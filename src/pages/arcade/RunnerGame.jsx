...
const PLAYER_X = 70;
const OBSTACLES = [
  { kind: 'rock', w: 30 }, { kind: 'cactus', w: 26 }, { kind: 'log', w: 34 }, { kind: 'spike', w: 28 },
];
const POWERUPS = [
  { kind: 'shield', emoji: '🛡️' },
  { kind: 'magnet', emoji: '🧲' },
...
export default function RunnerGame() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      c.save(); c.translate(330, 60);
      if (isNight) drawMoon(c); else drawSun(c, 17, s.frame);
      c.restore();
      ...
      s.clouds.forEach((cl) => {
        ...
        drawCloudProp(c, 0.6 + cl.s * 0.5, 0.85);
        c.restore();
      });
      // Trees (dekat)
      s.trees.forEach((t) => {
        if (moving) { t.x -= spd * 0.7; if (t.x < -60) t.x += 660; }
        c.save(); c.translate(t.x, GROUND_Y + 6);
        drawTreeProp(c, 1.15);
        c.restore();
      });
      ...
      s.pickups.forEach((p) => {
        ...
        if (p.kind === 'coin') drawCoin(c, s.frame + p.x);
        else if (p.kind === 'token') drawTokenBadge(c, p.token.emoji, s.frame + p.x);
        else drawPowerBadge(c, p.power.kind, s.frame + p.x);
        c.restore();
      });

      // ── OBSTACLES ──
      s.obstacles.forEach((o) => {
        c.save(); c.translate(o.x, GROUND_Y + 20);
        (OB_DRAW[o.kind] || drawRock)(c, s.frame);
        c.restore();
      });
      ...
      c.translate(0, -10);
      drawFox(c, s.frame, moving && !s.jumping);
      c.restore();
      // Shield bubble
      if (s.shield) {
      ...
      <div className="absolute inset-0 flex items-center justify-center" onPointerDown={jump}>
        ...
        {!started && !gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-slate-950/60 backdrop-blur-sm pointer-events-none">
            <CharacterCanvas draw={drawFox} className="mb-3 drop-shadow-2xl" />
