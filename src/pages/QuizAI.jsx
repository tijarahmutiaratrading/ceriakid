...
export default function QuizAI() {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="relative max-w-5xl mx-auto px-5 sm:px-8 lg:px-10 pt-4 pb-16">
        <div className="mb-4">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-5 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3">
            <img
              src="https://media.base44.com/images/public/69f1c132ffcd7c660466eec5/4347879e0_generated_image.png"
              alt="Cikgu Rosie"
        ...
        {score.total > 0 && (
          <motion.div
            ...
            className="mb-4 bg-white/80 backdrop-blur-md border border-slate-200 shadow-sm rounded-2xl p-3 flex items-center justify-between gap-3"
          >
            <div className="flex items-center gap-2">
              <Trophy className="w-5 h-5 text-amber-500" />
              <div>
                <p className="text-slate-600 text-[10px] font-black uppercase tracking-widest">Skor anda</p>
                <p className="text-slate-900 font-black text-base">{score.correct} / {score.total} betul ({accuracy}%)</p>
              </div>
            </div>
            <button onClick={handleReset} className="px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs flex items-center gap-1.5 transition-all">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/80 backdrop-blur-md border border-slate-200 shadow-sm rounded-3xl p-5 mb-4 space-y-4">
                ...
                {SUBJECTS.map(s => (
                  <button
                    ...
                    className={`px-2 py-2.5 rounded-xl text-xs font-black transition-all ${
                      subject === s.value ? 'bg-gradient-to-br from-cyan-500 to-indigo-600 text-white shadow' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                    }`}
                  >
                ...
                {LEVELS.map(l => (
                  <button
                    ...
                    className={`flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-black transition-all ${
                      level === l.value ? 'bg-gradient-to-br from-cyan-500 to-indigo-600 text-white shadow' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                    }`}
                  >
                    {l.label}
                  </button>
              ...
              <div className="grid grid-cols-3 gap-2">
                {DIFFICULTIES.map(d => (
                  <button
                    ...
                    className={`px-2 py-2.5 rounded-xl text-xs font-black transition-all ${
                      difficulty === d.value ? 'bg-gradient-to-br from-cyan-500 to-indigo-600 text-white shadow' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
                    }`}
                  >
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400 text-sm font-semibold focus:outline-none focus:bg-white focus:border-cyan-400"
              />
            </div>
        ...
        {/* Loading state */}
        {loading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/80 backdrop-blur-md border border-slate-200 shadow-sm rounded-3xl p-8 text-center">
            <Loader2 className="w-12 h-12 text-cyan-500 animate-spin mx-auto mb-3" />
            <p className="text-slate-900 font-black text-base">AI sedang jana soalan...</p>
            <p className="text-slate-600 text-xs mt-1">Sila tunggu sebentar 🎯</p>
          </motion.div>
        )}
          ...
          {currentQuiz && !loading && (
            ...
            >
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                ...
                <button
                  onClick={handleReset}
                  className="py-3 rounded-2xl bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-black text-sm transition-all flex items-center justify-center gap-1.5 shadow-sm"
                >
