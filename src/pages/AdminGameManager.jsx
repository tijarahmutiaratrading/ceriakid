...
export default function AdminGameManager() {
  ...
  const handleQueueMiniGames = async () => {
    if (selectedMiniGames.size === 0) { showToast('Pilih sekurang-kurangnya satu mini game', false); return; }
    ...
    try {
      ...
      showToast(`✅ ${selectedMiniGames.size} mini game tasks dihantar ke queue!`);
        ...
        {tab === 'generator' && (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <div className="p-6 rounded-3xl mb-5" style={{ background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.2)' }}>
              ...
              <div className="mb-5">
                <label className="text-white/70 text-xs font-black uppercase tracking-wider block mb-2">📝 Soalan per Game</label>
                <input type="number" min="1" max="50" value={miniGameConfig.questions}
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex gap-2 items-center">
                  <button onClick={() => setSelectedMiniGames(new Set(GAME_HUB.map(g => g.id)))} className="text-xs font-bold text-yellow-300 hover:underline">Semua</button>
                  <span className="text-white/30">|</span>
                  <button onClick={() => setSelectedMiniGames(new Set())} className="text-xs font-bold text-white/50 hover:underline">Kosong</button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-5">
                {GAME_HUB.map(game => {
                  ...
                  return (
                    <button key={game.id} onClick={() => toggleMiniGame(game.id)}
                      className={`flex items-start gap-2 px-3 py-2.5 rounded-xl text-xs font-bold transition-all text-left ${sel ? 'bg-white text-indigo-700 shadow-lg' : 'bg-white/10 text-white/70 hover:bg-white/20'}`}>
                      <div className={`w-4 h-4 rounded border-2 flex-shrink-0 mt-0.5 flex items-center justify-center ${sel ? 'bg-indigo-600 border-indigo-600' : 'border-white/30'}`}>
                        {sel && <span className="text-white text-xs">✓</span>}
                      </div>
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                ...
                {miniGameSubmitting ? <><Loader2 className="w-5 h-5 animate-spin" /> Menghantar ke queue...</> : <><Wand2 className="w-5 h-5" /> Queue Mini Games ({selectedMiniGames.size})</>}
              </motion.button>
            </div>
            </motion.div>
            )}
