 ...
 * Sticky di bawah canvas. Satu tap = popover (tools / size / colors / stickers).
 * Tujuan: anak tak perlu scroll bawah cari panel — semua dalam reach.
 */
export default function CanvasFloatingToolbar({
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      >
        {/* TOOL */}
        <PopoverButton active={openPopover === 'tool'} onClick={() => togglePopover('tool')} label={tool.label}>
          <span className="text-xl leading-none">{tool.emoji}</span>
        </PopoverButton>

        {/* SIZE */}
        <PopoverButton active={openPopover === 'size'} onClick={() => togglePopover('size')} label={brushSize.label}>
          <span className="inline-block rounded-full" style={{ width: brushSize.dot, height: brushSize.dot, backgroundColor: '#0f172a' }} />
        </PopoverButton>

        {/* COLOR */}
        {tool.id !== 'eraser' && (
          <PopoverButton active={openPopover === 'color'} onClick={() => togglePopover('color')} label="Warna">
        ...
        {/* STICKER */}
        {showStickers && (
          <PopoverButton active={openPopover === 'sticker' || !!stickerMode} onClick={() => togglePopover('sticker')} label="Sticker">
      ...
      <AnimatePresence>
        ...
        {openPopover === 'color' && tool.id !== 'eraser' && (
          <Popover>
        ...
        {openPopover === 'sticker' && showStickers && (
          <Popover>
...
function PopoverButton({ active, onClick, label, children }) {
  return (
    <button
      type="button"
      onClick={onClick}
      title={label}
      className={`relative flex flex-col items-center justify-center gap-0.5 w-12 sm:w-14 h-10 rounded-full transition ${active ? 'bg-slate-900 text-white shadow-md' : 'text-slate-700 hover:bg-slate-100'}`}
    >
...
function Popover({ children }) {
  return (
    <motion.div
      ...
      className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 z-30 w-[min(92vw,360px)] p-3 rounded-2xl"
      style={{
