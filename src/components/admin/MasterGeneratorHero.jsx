...
const StatCard = ({ icon: Icon, label, value, tone }) => (
  <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-white/10 p-4 shadow-xl shadow-black/10">
    <div className={`absolute -right-5 -top-5 h-20 w-20 rounded-full blur-2xl ${tone}`} />
    <div className="relative flex items-center justify-between gap-3">
      <div>
        <p className="text-2xl font-black text-white leading-none">{value}</p>
        <p className="mt-1 text-[11px] font-black uppercase tracking-wider text-white/55">{label}</p>
      </div>
...
export default function MasterGeneratorHero({ totalGames, totalFull, totalPlayers, pendingTasks, runningTasks, failedTasks }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[2.25rem] border border-white/20 bg-white/10 p-5 shadow-2xl shadow-fuchsia-950/30 backdrop-blur-3xl md:p-7">
      ...
      <div className="relative grid gap-6 lg:grid-cols-[1.2fr_1fr] lg:items-end">
        <div className="space-y-5">
          ...
          <div className="flex items-start gap-4">
            <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-[1.65rem] bg-gradient-to-br from-yellow-300 via-orange-400 to-pink-500 text-3xl shadow-2xl shadow-purple-950/30 ring-1 ring-white/30 md:h-20 md:w-20 md:text-4xl">
              🎮
            </div>
            <div className="min-w-0">
              <h1 className="text-3xl font-black tracking-tight text-white md:text-5xl">Master Generator</h1>
              <p className="mt-2 max-w-2xl text-sm font-semibold leading-relaxed text-white/70 md:text-base">
