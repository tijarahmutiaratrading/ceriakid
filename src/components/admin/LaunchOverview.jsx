...
function ProgressCard({ icon: Icon, title, accent, current, target, percent, enabled, interval, status, extra }) {
  const accents = {
    violet: { ring: 'ring-violet-200', text: 'text-violet-600', bar: 'from-violet-400 to-purple-500', badge: 'bg-violet-100 text-violet-800' },
    sky:    { ring: 'ring-sky-200',    text: 'text-sky-600',    bar: 'from-sky-400 to-blue-500',      badge: 'bg-sky-100 text-sky-800' },
    amber:  { ring: 'ring-amber-200',  text: 'text-amber-600',  bar: 'from-amber-400 to-orange-500',  badge: 'bg-amber-100 text-amber-800' },
  };
  const a = accents[accent];
  return (
    <div className={`rounded-2xl p-4 bg-white ring-1 ${a.ring} shadow-sm`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <Icon className={`w-5 h-5 ${a.text}`} />
          <span className="text-slate-900 font-black text-sm">{title}</span>
        </div>
        {enabled !== undefined && (
          <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${enabled ? 'bg-emerald-500 text-white' : `${a.badge}`}`}>
      ...
      <p className="text-[11px] font-bold text-slate-600">{percent}% complete{interval && <span className="text-slate-400"> · {interval}</span>}</p>
...
function StatPill({ label, value, color, sub }) {
  ...
  return (
    <div className="rounded-xl p-3 bg-white ring-1 ring-slate-200 shadow-sm">
  ...
  if (loading) {
    return (
      <div className="pro-glass rounded-3xl p-12 text-center">
        ...
        <p className="text-slate-700 font-bold text-sm">Memuatkan status...</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="pro-glass rounded-3xl p-6 text-center">
        ...
        <button onClick={fetch} className="mt-3 px-4 py-2 bg-slate-900 text-white rounded-xl font-bold text-sm">Cuba lagi</button>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="pro-glass rounded-2xl p-3 flex items-center justify-between gap-3 flex-wrap">
        <p className="text-slate-700 text-xs font-semibold">
          ...
          {autoRefresh && <span className="text-emerald-600 ml-2">· auto 60s</span>}
        </p>
        <div className="flex gap-2">
          <button
            onClick={() => setAutoRefresh(!autoRefresh)}
            className={`px-3 py-1.5 rounded-lg text-[11px] font-black ring-1 transition-all ${
              autoRefresh ? 'bg-emerald-100 text-emerald-700 ring-emerald-300' : 'bg-white text-slate-700 ring-slate-300'
            }`}
          >
            {autoRefresh ? '⏸ Pause' : '▶ Auto'}
          </button>
          <button onClick={fetch} disabled={refreshing} className="px-3 py-1.5 rounded-lg bg-white hover:bg-slate-50 text-slate-700 text-[11px] font-black ring-1 ring-slate-300 transition-all disabled:opacity-60 flex items-center gap-1">
        ...
        <ProgressCard
          ...
          extra={kssr.nextBucket && (
            <div className="p-2 rounded-lg bg-amber-50 ring-1 ring-amber-200">
              <p className="text-[10px] font-black text-amber-800">Next: {kssr.nextBucket.ageGroup}{kssr.nextBucket.darjah ? `/${kssr.nextBucket.darjah}` : ''} → {kssr.nextBucket.category}</p>
              <p className="text-[10px] text-slate-600">Perlu lagi {kssr.nextBucket.needed} games</p>
            </div>
          )}
        />
        <ProgressCard
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className={`rounded-xl p-3 ring-1 shadow-sm ${isHot ? 'bg-emerald-100 ring-emerald-300' : 'bg-white ring-slate-200'}`}>
          <p className={`text-[10px] font-black uppercase tracking-wider mb-1 ${isHot ? 'text-emerald-700' : 'text-slate-500'}`}>
            {isHot ? '🔥 Live' : 'Last 5 min'}
          </p>
          <p className={`text-2xl font-black ${isHot ? 'text-emerald-700' : 'text-slate-400'}`}>{activity.createdLast5Min}</p>
          <p className="text-[10px] text-slate-500 mt-0.5">games created</p>
        </div>
      ...
      {activity.recent?.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="pro-glass rounded-2xl p-4">
          <h3 className="text-slate-900 font-black text-sm mb-3 flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-emerald-500" /> Games Terkini Dicipta
          </h3>
          <div className="space-y-1.5">
            {activity.recent.map(g => {
              ...
              return (
                <div key={g.id} className="rounded-lg p-2.5 bg-emerald-50 ring-1 ring-emerald-200 flex items-center justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <p className="text-slate-900 font-bold text-xs truncate">{g.title}</p>
                    <p className="text-[10px] text-slate-600">{lvl} · {subj}</p>
                  </div>
                  <span className="text-[10px] font-black text-emerald-700 bg-emerald-100 px-2 py-0.5 rounded-full whitespace-nowrap">
                    {timeAgo(g.createdAt)}
                  </span>
                </div>
              );
