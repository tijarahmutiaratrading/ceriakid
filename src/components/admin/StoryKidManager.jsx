...
export default function StoryKidManager({ onToast }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-3 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] shadow-2xl shadow-black/20" style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.14), rgba(255,255,255,0.06))', backdropFilter: 'blur(24px)', border: '1px solid rgba(255,255,255,0.22)' }}>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
        <div className="min-w-0">
          <h2 className="font-black text-white text-xl sm:text-2xl leading-tight">Story Kid Management</h2>
          <p className="text-white/60 text-xs sm:text-sm mt-1">Urus cerita, status publish dan kandungan Story Kid.</p>
        </div>
        <div className="flex items-center gap-2 justify-end sm:justify-start flex-wrap">
          {stories.length > 0 && (
            confirmDeleteAll ? (
              <div className="flex items-center gap-2 px-2 sm:px-3 py-2 rounded-2xl bg-red-500/20 border border-red-400/20">
                <span className="text-red-100 font-black text-[11px] sm:text-xs whitespace-nowrap">Confirm?</span>
              ...
              <button onClick={() => setConfirmDeleteAll(true)} disabled={loading} className="flex items-center gap-2 px-3 py-2.5 sm:py-3 rounded-2xl bg-red-500/20 hover:bg-red-500/30 text-red-200 font-black text-xs transition-all">
      ...
      {loading ? (
        ...
        <div className="text-center py-12 px-5 rounded-3xl bg-white/10 border border-white/10">
          ...
          <button onClick={importDefaultStories} className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-3 rounded-2xl bg-white text-purple-700 font-black shadow-xl">
        ...
        <div className="space-y-3">
          {stories.map((story) => {
            ...
            return (
              <div key={story.id} className="rounded-3xl p-3 sm:p-4 bg-white/10 border border-white/15 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                <div className="flex items-start gap-3 flex-1 min-w-0 w-full">
                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-white/15 flex items-center justify-center text-2xl sm:text-3xl flex-shrink-0">{story.emoji || '📖'}</div>
                ...
                <div className="grid grid-cols-4 sm:flex sm:items-center gap-2 flex-shrink-0 w-full sm:w-auto">
                  <Link to="/story-kid" className="p-2.5 rounded-xl bg-white/10 text-white hover:bg-white/20 flex items-center justify-center"><BookOpen className="w-4 h-4" /></Link>
                  <button onClick={() => setEditStory(story)} className="p-2.5 rounded-xl bg-blue-500/20 text-blue-200 hover:bg-blue-500/30 flex items-center justify-center"><Edit3 className="w-4 h-4" /></button>
                  <button onClick={() => togglePublish(story)} className="px-2 sm:px-3 py-2.5 rounded-xl bg-white/10 text-white text-xs font-black hover:bg-white/20">{story.isPublished ? 'Hide' : 'Show'}</button>
                  <button onClick={() => deleteStory(story)} className="p-2.5 rounded-xl bg-red-500/20 text-red-200 hover:bg-red-500/30 flex items-center justify-center"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            );
          })}
