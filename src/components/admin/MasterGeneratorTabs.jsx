...
export default function MasterGeneratorTabs({ tab, setTab, miniGamesTab, setMiniGamesTab, storyKidTab, setStoryKidTab }) {
  ...
  return (
    <div className="sticky top-24 z-30 rounded-[2rem] border border-white/20 bg-white/10 p-2 shadow-2xl shadow-purple-950/25 backdrop-blur-3xl">
      <div className="flex gap-2 overflow-x-auto scroll-px-2 pb-0.5" style={{ WebkitOverflowScrolling: 'touch' }}>
        {tabs.map((item) => (
          <button
            key={item.id}
            type="button"
            onClick={item.onClick}
            className={`group relative flex min-w-[8.75rem] flex-none flex-col items-start overflow-hidden rounded-[1.35rem] px-4 py-3 text-left transition-all duration-200 lg:min-w-0 lg:flex-1 ${item.active ? 'bg-white text-indigo-900 shadow-xl shadow-black/15' : 'text-white/70 hover:bg-white/12 hover:text-white'}`}
          >
            {item.active && <div className="absolute inset-0 bg-gradient-to-br from-white via-white to-indigo-50" />}
            <span className="relative z-10 text-[10px] font-black uppercase tracking-wider opacity-60">{item.section}</span>
            <span className="relative z-10 mt-1 flex items-center gap-2 text-xs font-black md:text-sm">
