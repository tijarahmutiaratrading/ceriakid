...
export default function CinematicShowcase({ item, playLabel, onPlay, fit = 'cover' }) {
  return (
            ...
            <button onClick={onPlay} className="relative block w-full overflow-hidden rounded-[1.75rem] border border-white/15 shadow-2xl group text-left">
              {item.art ? (
                <img
                  src={item.art}
                  alt={item.title}
                  className={`w-full aspect-[16/10] ${fit === 'contain' ? 'object-contain p-2' : 'object-cover'} group-hover:scale-[1.04] transition-transform duration-700`}
                />
              ) : (
                <div
