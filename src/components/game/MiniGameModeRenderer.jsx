...
function TypingMode({ data: rawData }) { const data = useRoundData(rawData); const [value, setValue] = useState(''); const ok = normalize(value) === normalize(data.target); return <div className="space-y-4"><MiniScore score={ok ? 1 : 0} total={1} /><div className={`${panel} text-center`}><p className="text-white/70 text-xs font-black">Taip perkataan</p><p className="text-5xl font-black text-yellow-300 my-3">{data.target}</p><input value={value} onChange={e => setValue(e.target.value)} className="w-full rounded-2xl p-4 text-center font-black text-2xl text-purple-700" autoFocus /></div><p className="text-center text-white font-black">{ok ? 'Hebat! 🚗💨' : 'Terus cuba!'}</p></div>; }
