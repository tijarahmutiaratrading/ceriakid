...
function PictureHuntMode({ data }) { const { feedback, showFeedback } = useMiniFeedback(); const [score, setScore] = useState(0); const items = data.items || []; const pick = (item) => { const ok = isSame(item, data.target); showFeedback(ok ? 'correct' : 'wrong', ok ? 'Jumpa sasaran!' : 'Cuba cari yang lain.'); if (ok) setScore(1); }; return <div className="space-y-4"><MiniFeedback feedback={feedback} /><MiniScore score={score} total={1} /><div className={panel}><p className="text-white font-black text-center">Cari: <span className="text-yellow-300">{data.target}</span></p></div><div className="grid grid-cols-2 gap-3">{items.map(item => <button key={item} onClick={() => pick(item)} className="aspect-square rounded-3xl bg-white/20 border border-white/30 text-4xl font-black text-white">{item}</button>)}</div></div>; }
