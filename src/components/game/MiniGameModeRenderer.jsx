...
function SortingMode({ data }) {
  ...
  return <div className="space-y-4"><MiniFeedback feedback={feedback} /><MiniScore score={used.length} total={items.length} /><div className={panel}><p className="text-white/70 text-xs font-black mb-3 uppercase">Pilih item</p><div className="flex flex-wrap gap-2">{items.filter(item => !used.includes(item.text)).map(item => <button key={item.text} draggable onClick={() => setSelected(item)} onDragStart={() => { dragItem.current = item; }} className={`px-4 py-3 rounded-2xl font-black ${selected?.text === item.text ? 'bg-yellow-300 text-purple-900' : 'bg-white text-purple-700'}`}>{item.text}</button>)}</div></div><div className="grid grid-cols-2 gap-3">{groups.map(group => <button key={group} onClick={() => place(group)} onDragOver={e => e.preventDefault()} onDrop={() => place(group)} className={`${panel} min-h-40 text-left`}><p className="text-white font-black mb-3">{group}</p>{(sorted[group] || []).map(item => <div key={item.text} className="mb-2 px-3 py-2 rounded-xl bg-white/25 text-white font-bold">{item.text}</div>)}</button>)}</div></div>;
}
...
function PictureHuntMode({ data }) { const { feedback, showFeedback } = useMiniFeedback(); const [score, setScore] = useState(0); const items = data.items || []; const pick = (item) => { const ok = isSame(getItemValue(item), data.target) || isSame(getItemText(item), data.target); showFeedback(ok ? 'correct' : 'wrong', ok ? 'Jumpa sasaran!' : 'Cuba cari gambar lain.'); if (ok) setScore(1); }; return <div className="space-y-4"><MiniFeedback feedback={feedback} /><MiniScore score={score} total={1} /><div className={panel}><p className="text-white font-black text-center">Cari gambar untuk: <span className="text-yellow-300">{data.target}</span></p></div><div className="grid grid-cols-2 gap-3">{items.map((item, i) => <button key={`${getItemText(item)}-${i}`} onClick={() => pick(item)} className="aspect-square rounded-3xl bg-white/20 border border-white/30 text-5xl font-black text-white">{getItemText(item)}</button>)}</div></div>; }
...
function MiniSimulationMode({ data }) { const { feedback, showFeedback } = useMiniFeedback(); const [picked, setPicked] = useState([]); const items = data.items || []; const total = items.filter(item => isSame(getItemGroup(item), data.target)).length || 1; const pick = (item, i) => { const ok = isSame(getItemGroup(item), data.target); showFeedback(ok ? 'correct' : 'wrong', ok ? 'Objek betul!' : 'Objek ini tidak sesuai.'); if (ok && !picked.includes(i)) setPicked(prev => [...prev, i]); }; return <div className="space-y-4"><MiniFeedback feedback={feedback} /><MiniScore score={picked.length} total={total} /><div className={`${panel} text-center`}><p className="text-6xl mb-2">🧲</p><p className="text-white font-black">Tap objek yang tertarik kepada magnet</p></div><div className="grid grid-cols-3 gap-2">{items.map((item, i) => <button key={`${getItemText(item)}-${i}`} type="button" onClick={() => pick(item, i)} className={`${chip} ${picked.includes(i) ? 'opacity-40' : ''}`}>{getItemText(item)}</button>)}</div></div>; }
