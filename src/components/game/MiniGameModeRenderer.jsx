...
function BalloonPopMode({ data }) {
  const [score, setScore] = useState(0); const items = data.items || [];
  return <div className="space-y-4"><MiniScore score={score} total={items.filter(x => x === data.target).length || 1} /><div className={`${panel} text-center`}><p className="text-white font-black">Pop huruf: <span className="text-yellow-300 text-3xl">{data.target}</span></p></div><div className="relative h-80 rounded-3xl bg-sky-300/25 border border-white/20 overflow-hidden">{items.map((item, i) => <motion.button key={i} initial={{ y: 230 }} animate={{ y: [230, 20, 230], x: [0, i % 2 ? 30 : -20, 0] }} transition={{ repeat: Infinity, duration: 3 + i * 0.4 }} onClick={() => item === data.target && setScore(s => s + 1)} className="absolute text-5xl" style={{ left: `${12 + (i * 17) % 72}%` }}>🎈<span className="absolute inset-0 flex items-center justify-center text-base font-black text-white pb-2">{item}</span></motion.button>)}</div></div>;
}

function FallingCatchMode({ data }) { const [score, setScore] = useState(0); return <div className="space-y-4"><MiniScore score={score} total={2} /><div className={`${panel} text-center`}><p className="text-white font-black">Tangkap nombor {data.target}</p></div><div className="h-80 rounded-3xl bg-blue-300/20 relative overflow-hidden border border-white/20">{(data.items || []).map((item, i) => <motion.button key={i} initial={{ y: -40 }} animate={{ y: 270 }} transition={{ repeat: Infinity, duration: 2.5 + i * 0.25, delay: i * 0.3 }} onClick={() => item === data.target && setScore(s => s + 1)} className="absolute w-14 h-14 rounded-full bg-white text-purple-700 font-black text-xl" style={{ left: `${10 + i * 16}%` }}>{item}</motion.button>)}<div className="absolute bottom-2 left-1/2 -translate-x-1/2 text-5xl">🧺</div></div></div>; }
...
function MiniSimulationMode({ data }) { const [score, setScore] = useState(0); return <div className="space-y-4"><MiniScore score={score} total={2} /><div className={`${panel} text-center`}><p className="text-6xl mb-2">🧲</p><p className="text-white font-black">Tap objek yang tertarik kepada magnet</p></div><div className="grid grid-cols-3 gap-2">{(data.items || []).map(item => <button key={item.text} onClick={() => item.group === data.target && setScore(s => s + 1)} className={chip}>{item.text}</button>)}</div></div>; }
