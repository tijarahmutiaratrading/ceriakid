...
Deno.serve(async (req) => {
  try {
    ...
    const data = await base44.asServiceRole.integrations.Core.InvokeLLM({
      prompt: `Jana ${typeLabel} lengkap untuk ${subjectLabel} ${levelLabel}. Topik: ${topic || 'umum'}. Bilangan item/soalan: ${count}. Sesuai KSSR Malaysia, penuh, bukan placeholder, siap print A4. Pulangkan JSON: title, description, instructions, items array dengan heading, content, answer.`,
